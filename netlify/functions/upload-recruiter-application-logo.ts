...
function parseMultipart(event: any): Promise<UploadedLogo> {
  return new Promise((resolve, reject) => {
    ...
    if (!contentType?.includes("multipart/form-data")) {
      reject(new ApplicationError(400, "Expected a company logo image upload."));
      return;
    }
    ...
    parser.on("file", (_field, stream, info) => {
      ...
      stream.on("limit", () => {
        fail(new ApplicationError(413, "Company logo size must be less than 2MB."));
        stream.resume();
      });
      stream.on("end", () => {
        if (settled || stream.truncated) return;
        logo = {filename: info.filename || "", buffer: Buffer.concat(chunks)};
      });
    });
    ...
    parser.on("finish", () => {
      ...
      if (!logo?.buffer.length) {
        fail(new ApplicationError(400, "Choose a PNG, JPG, or WebP company logo."));
        return;
      }
      settled = true;
      resolve(logo);
    ...
    try {
      ...
      fail(new ApplicationError(400, "The company logo upload could not be read."));
    }
  });
}
...
export const handler: Handler = async event => {
  ...
  try {
    ...
    if (!token) throw new ApplicationError(401, "Please sign in before uploading a company logo.");
    ...
    const rateLimit = await checkRateLimit({
      admin,
      action: "recruiter-application-logo-upload",
    ...
    const storagePath =
      `recruiter-application-logos/${decoded.uid}/application-logo-${Date.now()}-${randomUUID()}.${detectedType}`;
    ...
    await bucket.file(storagePath).save(upload.buffer, {
      ...
      metadata: {
        ...
        metadata: {
          ...
          uploadPurpose: "recruiter-application",
          validatedBy: "upload-recruiter-application-logo",
        },
      },
    });
    ...
    if (uploadedPath) {
      try {
        ...
        // Cleanup is best-effort and does not expose storage details.
      }
    }
    ...
    console.error(
      "UPLOAD_RECRUITER_APPLICATION_LOGO_ERROR",
      error instanceof Error ? error.message : "Unknown logo upload error",
    );
    return json(500, origin, {error: "The company logo could not be uploaded. Please try again."});
  }
};
