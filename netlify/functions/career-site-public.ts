...
import {getCachedPublishedJobsForCompany, getCachedPublicCareerSite, resolveCompanyFromHost} from "./_careerSite";
...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    const cachedTenant = await getCachedPublicCareerSite(db, tenant.config.slug) || tenant;
    const jobs = await getCachedPublishedJobsForCompany(db, cachedTenant.companyId, cachedTenant.config.slug);
